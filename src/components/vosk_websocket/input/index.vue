<template>
  <div
    class="fixed bottom-[50px] left-1/2 transform -translate-x-1/2 w-[90%] z-[3001]"
    id="input"
    @keyup.enter="submit"
  >
    <input
      ...
      class="block w-full text-md h-[70px] px-4 text-slate-900 bg-white rounded-[17px] border border-slate-200 appearance-none focus:border-transparent focus:outline focus:outline-2 focus:outline-primary focus:ring-0 hover:border-brand-500-secondary- peer invalid:border-error-500 invalid:focus:border-error-500 overflow-ellipsis overflow-hidden text-nowrap pr-[48px]"
      id="floating_outlined"
      type="text"
    />
    <div class="absolute top-6 right-3">
      <div v-html="props.icon"></div>
    </div>
  </div>
</template>
...
//当触发回车 或者点击发送事件的时候把数据返回到父级
const submit = () => {
  //把收集的输出返回给父级
  emit('submit-data', content.value)
}
</script>
