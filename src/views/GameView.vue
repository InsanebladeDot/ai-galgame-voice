<template>
  ...
  >
    <div class="absolute inset-0 z-0 w-full h-full overflow-hidden">
      <div
        class="absolute inset-0 z-20 pointer-events-none flex justify-center items-end transition-all duration-1000 opacity-100"
      >
        <div
          class="absolute left-1/2 transform -translate-x-1/2 transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1) z-20 opacity-100 translate-y-0 h-[95%] bottom-0 lg:h-[125vh] lg:-bottom-[15vh]"
        ...
        >
          <div class="relative h-full scale-100 animate-breathe transition-transform duration-500">
            <img
              :alt="roleStore.role?.name"
              class="h-full w-auto object-contain transition-all duration-500 ease-in-out"
              :src="CurConversation.CurConversation?.currentAction"
            />
          </div>
        </div>
      </div>
      ...
      <div
        class="absolute inset-0 z-50 pointer-events-none flex flex-col justify-end transition-all duration-1000 transform opacity-100 translate-y-0"
      >
        <div
          class="w-full p-2 md:p-6 lg:pb-8 lg:px-24 flex justify-center items-end font-sans pointer-events-none"
        >
          <ChatBox />
        </div>
      </div>
      ...
      <!-- 喇叭触发模仿声音 -->
      <VioceButton />
      <!-- 提供的volume 需要0 - 1 实际提供值是 0-100 -->
      <backgroundBGM :volume="systemSettingStore.volume" />
    </div>
  </div>
</template>
...
import backgroundBGM from '@/components/Load/game/Voice/BrackBGM.vue'
//导入全局音频设置
